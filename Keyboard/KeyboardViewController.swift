...
class KeyboardViewController: UIInputViewController {
    ...
    */
            ...
            for rowKeys in page.rows { // TODO: quick hack
                for key in rowKeys {
                    if let keyView = self.layout?.viewForKey(key) {
                        ...
                        keyView.addTarget(self, action: #selector(KeyboardViewController.hapticDown), forControlEvents: .TouchDown)
                        
                        keyView.addTarget(self, action: #selector(KeyboardViewController.hapticRelease), forControlEvents: .TouchUpInside)
                        
                        keyView.addTarget(self, action: #selector(KeyboardViewController.hapticDrag), forControlEvents: .TouchDragExit)

                        keyView.addTarget(self, action: #selector(KeyboardViewController.hapticRepeat), forControlEvents: .TouchDownRepeat)
    ...
    let keypressFeedback = UIImpactFeedbackGenerator(style: UIImpactFeedbackStyle.Light)
    let keyreleaseFeedback = UIImpactFeedbackGenerator(style: UIImpactFeedbackStyle.Light)
    let dragFeedback = UISelectionFeedbackGenerator()

    func hapticDown() {
        keypressFeedback.impactOccurred()
        keypressFeedback.prepare()
